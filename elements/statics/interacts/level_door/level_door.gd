...
func _ready():
	SignalBus.connect("next_ready", Callable(self, "_on_hub_start"))
	SignalBus.connect("load_next", Callable(self, "_on_level_load"))
	$Popup.visible = false
	disabled = true

func _unhandled_input(event: InputEvent) -> void:
	if inside and !disabled and event.is_action_pressed("interact"):
...
func _on_level_load():
	SignalBus.emit_signal("enter_level", level_data)
